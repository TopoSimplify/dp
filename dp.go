...
func (self *DP)  build(process func(*bst.Node)) *DP {
    ...
    var rnge = item.NewInt2D(0, len(self.pln) - 1)
    ...
    var root = bst.NewNode(NewDPNode(rnge))
    self.BST.Root = root
    ...
    for !stack.IsEmpty() {
        ...
        if !math.IsNaN(val) && val <= self.res {
            intset.Add(item.Int(rnge[0]))
            intset.Add(item.Int(rnge[1]))
        } else {
            //left and right branch
            dpLnode = NewDPNode(item.NewInt2D(rnge[0], index))
            dpRnode = NewDPNode(item.NewInt2D(index, rnge[1]))
